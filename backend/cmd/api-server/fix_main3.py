...
if idx >= 0:
    # Find the closing brace and the log line
    # We need to find the pattern: }\n\n\tlog.Println(...)
    pattern = r'(\}\s*\n\s*log\.Println\("\[EMQX\] subscribed Feiyan-style topics: /sys/\+/+/prop/post, /sys/\+/+/thing/event/property/batch_post, /sys/\+/+/thing/event/property/history_post, /ota/device/inform/\+/+, /sys/\+/+/event/post, /sys/\+/+/service/+/reply, /sys/\+/+/rrpc/request/\+"\))'
    match = re.search(pattern, content)
    if match:
        print("Found pattern")
        old = match.group(0)
        print("Old:", repr(old[:200]))

...
if idx >= 0:
    ...
    if start >= 0:
        ...
        print('Found at start:', start)
        
        # The exact old string
        old_str = content[start:idx+150]
        print('Old string length:', len(old_str))
...
\tlog.Println("[EMQX] subscribed Feiyan-style topics: /sys/+/+/prop/post, /sys/+/+/thing/event/property/batch_post, /sys/+/+/thing/event/property/history_post, /ota/device/inform/+/+, /sys/+/+/event/post, /sys/+/+/service/+/reply, /sys/+/+/rrpc/request/+, /sys/+/+/thing/register")'''
        ...
        if old_str in content:
            ...
            print('Old string not found in content')
            print('Old str (first 200):', repr(old_str[:200]))
else:
    print('Not found')